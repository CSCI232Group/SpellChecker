...
public class SpellChecker {  
    ...
    final static char[] punctuation = "!.,;:?".toCharArray();
    static char punc = ' ';
    
    static void setPunctuation(char input){
        punc = input;
    }
    
    static private char getPunctuation(){
        return punc;
    }
    //addCHar
    ...
    private static void addtoList(ArrayList<String> returnList, String test, String input){
        if(returnList.contains(test)== false){
            if(input.length() <= test.length() + 2 )    //words need to be within 3 chars of original word
    ...
    private static String checkForPunctuation(String input){
        int lengthOfString = input.length() -1;
        char lastChar = input.charAt(lengthOfString);
       // StdOut.println("lastChar = " + lastChar);     //used for punctuation
        for(char a : punctuation){
            if(lastChar == a){
                input = input.substring(0, lengthOfString);
                StdOut.println("Punctuation found, word2 = " + input);
                setPunctuation(a);
                return input;
            }
        }
    ...
    public static String printListChoice(ArrayList<String> list){
        ...
        if(userIn == 0){
            ...
            //StdOut.println("Your chose: " + userIn  + " -> " +  userInput);   //used for debugging purposes
            return userInput;
        }else if(userIn == -1){
            StdOut.println("Problem");
            
        }else{
            //StdOut.println("Your chose: " + userIn + " -> " + list.get(userIn-1));  //used for debugging purposes
        ...
        try(FileWriter fw = new FileWriter("mydoc-checked.txt", true);
            ...
            PrintWriter out = new PrintWriter(bw))
            {
                ...
                while (!document.isEmpty()) {
                    ...
                    tempWord2 = checkForPunctuation(tempWord2);
                    punc = getPunctuation();
                    ...
                    if(inDictionary == true){
                        ...
                        //StdOut.println("The word, " +word2+" is in the dictionary" );  //used for debuging 
                        if(punc == ' '){                        //Check to see if puctuation was found.
                            out.print(tempWord2 + " ");
                                if(count%17 == 0){
                                    out.println("");
                            ...
                            out.print(word2 + punc + " ");
                                if(count%17 == 0){
                                    out.println("");
                        
                                }
                        }
                    }else{  //-------------------------------WORD SPELLED INCORRECTLY -------------------------------------
                        ...
                        if(punc == ' '){                                        //check for punctuation 
                            ...
                            out.print(correctedSpelling + punc + " ");          //print punctuaion if found.
                                if(count%17 == 0){
                                    out.println("");
